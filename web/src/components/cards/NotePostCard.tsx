...
const NotePostCard: React.FC<Props> = ({ data }) => {
  if (!data) {
    return null;
  }
  ...
  return (
    <Container>
      {title ? (
        <Link href={`/notes/${encodeURIComponent(slug)}`}>
          <a>
            <Title>{title}</Title>
          </a>
        </Link>
      ...
      <Body>{excerpt}</Body>
      <Link href={`/notes/${encodeURIComponent(slug)}`} passHref>
        <ButtonLink>
          <span>Read more</span>
          <FaLongArrowAltRight />
        </ButtonLink>
